...
// 1. Register — TIDAK kirim wallet (sesuai docs resmi)
...
// 2. Update wallet — endpoint baru dari docs resmi, WAJIB untuk airdrop
async function apiUpdateWallet(agentId, publicKey, walletAddress) {
  const resp = await fetch(`${BASE_URL}/update-wallet`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      agent_id: agentId,
  ...
  let data; try { data = JSON.parse(text); } catch { data = { raw: text }; }
  if (resp.status === 200) return { success: true, data };
  throw new Error(`Update wallet failed (${resp.status}): ${JSON.stringify(data)}`);
}
...
// 5. Check allocations
async function apiGetAllocation(agentId) {
  const resp = await fetch(`${BASE_URL}/allocations/${agentId}`, {
    method: 'GET',
    signal: AbortSignal.timeout(15000)
  });
  if (resp.status === 200) return await resp.json();
...
async function runAgent(account, config) {
  ...
  if (!privKey) {
    ...
    try {
      ...
      log(`❌ Invalid private key, generating new...`, agentId, 'WARN');
      const kp = generateKeypair();
      if (!kp) { log(`❌ Failed to generate keypair`, agentId, 'ERROR'); return; }
      pubKey = kp.publicKey;
      privKey = kp.privateKey;
  ...
  if (!state.registered || keyChanged || process.env.FORCE_REGISTER === 'true') {
    try {
      ...
      state.wallet_updated = false; // reset wallet update setelah register ulang
      ...
      log(`⚠️ Continuing anyway...`, agentId, 'WARN');
    }
  } else {
    log(`✅ Agent already registered`, agentId);
  }

  // Update wallet — WAJIB untuk airdrop, sesuai docs resmi
  if (!state.wallet_updated && account.wallet && account.wallet.trim() !== '') {
    try {
      log(`💳 Updating wallet address for airdrop...`, agentId);
      const { data } = await withRetry(
        () => apiUpdateWallet(agentId, pubKey, account.wallet),
        5, 3000, 'UpdateWallet', agentId
      );
      state.wallet_updated = true;
      saveState(agentId, state);
      log(`✅ Wallet updated! Address: ${data.wallet_address || account.wallet}`, agentId);
    } catch (err) {
      log(`⚠️ Wallet update failed: ${err.message} — reward mungkin tidak masuk!`, agentId, 'WARN');
    }
  } else if (state.wallet_updated) {
    log(`✅ Wallet already updated`, agentId);
  }
  ...
  while (true) {
    ...
    if (needReRegister) {
      try {
        ...
        state.registered = true;
        state.wallet_updated = false;
        state.last_registered_key = pubKey;
        saveState(agentId, state);
        log(`✅ Re-registration successful`, agentId);
        needReRegister = false;

        // Update wallet lagi setelah re-register
        try {
          await withRetry(() => apiUpdateWallet(agentId, pubKey, account.wallet), 3, 3000, 'UpdateWallet', agentId);
          state.wallet_updated = true;
          saveState(agentId, state);
          log(`✅ Wallet re-updated`, agentId);
        } catch (err) {
          log(`⚠️ Wallet re-update failed: ${err.message}`, agentId, 'WARN');
        }
      } catch (err) {
